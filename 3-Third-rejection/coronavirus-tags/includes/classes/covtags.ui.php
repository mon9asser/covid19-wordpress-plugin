  ...
  class CovTags_UI extends CovTags_Request {
    ...
    public function Card_All_Countries ( $options = array() , $fields = array(), $style_class = null, $drak_mode = false ){
      ...
      <div class="covtags-table-container <?php echo ( true === $drak_mode )? esc_attr( 'datatable-dark-mode' ) :'';?>">
        ...
        <!-- Table Body -->
        <div id="<?php echo esc_attr( $tableId ); ?>" data-graph-type='<?php echo esc_attr( $data_graph ); ?>' class="covtags-table covtags-datatable-contents covtags-datatable-<?php echo esc_attr( $data_table_classes );?>">
                ...
                <div data-country-title="<?php echo esc_attr( $country_name ); ?>" data-canvas-id="<?php echo esc_attr( $counrty_slug . '-canvas' ); ?>" <?php echo esc_attr( $row_attributes ); ?>  style="<?php echo esc_attr( $hide_row ); ?>" class="cotags-row-container" >
                  ...
                  <div class="covtags-table-row">
                    ...
                    <div class='covtags-table-cell covtags-country-cell' style="<?php echo esc_attr( $case_one_cell );?>">
                      <?php if ( false !== $data_table_options['show_country_flag'] ): ?>
                        <img class='covtags-img-beside-text' src="<?php echo esc_url( $country->countryInfo->flag ); ?>" width="20">
                      <?php endif; ?>
                  ...
                  <div class="covtags-graph-container">
                    <div class="graph-main-content">
                      ...
                      <div class="data-table-mobile-version">
                        <?php foreach ( $data_fields as $field_key => $attributes ): ?>
                        <div class="cov-data-row">
                          <!-- Api Fields -->
                            <div class="covtags-box-cell">
                              <span><?php echo $covtags_labels[$field_key]; ?>:</span>
                            </div>
                            <div class="covtags-box-cell">
                              <span> <?php echo apply_filters( 'covtags_si_system_number_units', $country->{$field_key}, 'format_comma' ); ?> </span>
                            </div>
                        </div>
                        <?php endforeach; ?>
                      </div>
    ...
    public function covtags_render_map ( $attributes ){
      ...
      # Send equest To Get World Report
      ...
      <div class="svg-map-container <?php echo ( $is_dark_mode === true )? 'map-in-dark-mode' :''; ?>" id="<?php echo esc_attr( $id_map ); ?>">
        ...
        <!-- Render SVG Map -->
        <?php include ( COVTAGS_DIR . 'assets/img/render-map.php' ); ?>

        <!-- tooltips -->
        <div class="covtags-map-tooltip">
          <div class="countryInfo">
            <img alt='Country Flag' src="" width="20" />
            <span></span>
          </div>
    ...
    public function Show_Cases_Status_with_graph ( $attributes ){
        ...
        # Border Radius
        $rounded        = isset( $attributes['rounded'] )? $attributes['rounded'] : 0;
        ...
        ob_start();
        ?>
        <div style="border-radius: <?php echo esc_attr( $rounded); ?>px;" class="cases-status-card status-card-<?php echo esc_attr( $style ); ?> <?php echo ( $is_dark_mode === true )? esc_attr( 'cases-status-dark-mode' ):''; ?>">
            ...
            <?php
              ...
              if ( true !== $hide_title ){
                echo sprintf(
                  '<div class=\'covtags-widget-title\'><span>%1$s %2$s</span></div>',
                  $targIcon,
                  $attributes['title']
                );
              }
            ...
            <div class="covtags-container status-card-container">
              <?php if ( 'no' !== $graph ) {
                echo sprintf(
              ...
              <div class="covtags-percentage-data">
                ...
                <?php
                  ...
                  echo sprintf(
                    '<div><span>%1$s</span><span class=\'covtags-label-data\'>%2$s</span></div>',
                    $numericalValue,
                    $basic_data_label
                  );
                ?>
    ...
    public function Card_All_countries_tricker ( $attributes ){
      ...
      # Dark Mode
      $is_dark_mode = ( 'yes' === $attributes['dark_mode'] ) ? 'ticker_in_dark_mode' : '';

      # Position
      $position     = !in_array( $attributes['position'], array( 'top', 'bottom', 'normal' ) ) ? 'normal': $attributes['position'];

      # Data Provider ( Selected Field ) also decending by this field
      $basic_fields = array(
        'cases',
        'todayCases',
        'deaths',
      ...
      $field        =  !in_array( $attributes['field'], $basic_fields ) ? 'cases': $attributes['field'];

      # Ticker Speed
      $ticker_speed = !in_array( $attributes['ticker_speed'], array ( 'fast', 'medium', 'slow', 'default' ) )? 'normal': $attributes['ticker_speed'] ;
 
      # Ticker Text
      $ticker_text  = ( NULL === $attributes['ticker_text'] ) ? $covtags_labels[$field]: $attributes['ticker_text'];

      # Sorting By Target Field ( Choosed in data provider )
      uasort( $countries, function( $a, $b ) use ( $field ) {
        return ( $a->{$field} < $b->{$field} );
      });

      # Build Unique Ticker Id to prevent any conflict
      $ticker_id    =  "covtags-ticker-" . time() . '-' . mt_rand( 1, 1000 );

      # Render HTML also render js callback here to prevent unknown elements when hover in list also to send unique id
      ob_start();
      ?>
      <!-- Ticker Wrapper -->
      <div id="<?php echo esc_attr( $ticker_id ); ?>" class="tags-ticker-wrapper tags-ticker-<?php echo esc_attr( $position ); ?>-position <?php echo esc_attr( $is_dark_mode ); ?>">
        <div class="tags-ticker-data">

          <!-- Ticker Title -->
          <div class="tags-ticker-title">
            <?php echo $ticker_text; ?>
          </div>

          <!-- Ticker Body -->
          <div class="tags-ticker tags-ticker-<?php echo esc_attr( $ticker_speed );?>">

            <!-- Countries -->
            <?php foreach ( $countries as $country_key => $country ): ?>

              <div class="tags-ticker-item">

                <!-- Flag -->
                <span class="img-flag-data" style="background-image:url(<?php echo esc_url( $country->countryInfo->flag ); ?>);"></span>

                <!-- Country Data -->
                <div class="covtags-countryInfo">
                 <span class="cov-countryname">
                   <?php echo $country->country;?>
                 </span>
                 <span class="cov-cases-number">
                   <b> <?php echo apply_filters( 'covtags_si_system_number_units', $country->{$field}, 'format_comma' );?> </b>
                 </span>
               </div>
    ...
    // Standard Card ( All Countries )
    public function Standard_Card_Countries ( $attributes ){

      # Get All Required Vars
      global $covtags_labels, $iso_countries;

      # Init Unique Ids and Predefined Vars
      $unique_id      = 'countries-data-' . mt_rand( 10 , 100 ) . '-' . time();
      $livedata_id    = 'standard-live-data-' . mt_rand( 10 , 100 ) . '-' . time();

      # Required Api ( Countries and Globe Report )
      $countries      = $this->stored_options['list_of_countries'];
      $countries_data = $this->Countries();
      $wreport        = $this->World_Report();
      $wreport        = $wreport->data;
      $title          = $attributes['title'];
      $liveword       = $attributes['live_word'];

      # Store Default Values
      if( $title === null ){
        $title    = esc_html__( "Coronavirus", COVTAGS_TEXTDOMAIN );
      }
      if( $liveword === null ){
        $liveword = esc_html__( "Live", COVTAGS_TEXTDOMAIN );
      }
      $is_dark    = ( $attributes['dark_mode'] === 'yes' ) ? true: false;

      # Start Html +++
      ob_start();
      ?>
      <div class="covtags-standard-card-wrapper <?php echo ( $is_dark )? esc_attr( 'standard-dark-mode' ): ''; ?>">

        <!-- Standard Card Title -->
        <h3>
          <span class="covtags-live-contents">
            <?php echo $liveword; ?>
          </span>
          <span><?php echo $title; ?></span>
        </h3>

        <!-- Standard Card Select Country or Globe -->
        <div class="covtags-list-countries-data">

          <!-- Combo Box -->
          <select id="<?php echo esc_attr ( $unique_id ); ?>">

            <!-- Globe Option -->
            <option
              data-flag="<?php echo esc_attr(  COVTAGS_SRC . 'assets/img/bg-icon.png' ); ?>"
              value="<?php echo esc_attr( '0' ); ?>">
              <?php echo esc_html__( 'World Wide', COVTAGS_TEXTDOMAIN ) ; ?>
            </option>
            ...
            <option
              ...
              value="<?php echo esc_attr( $country['_id']); ?>">
              <?php echo $country['country']; ?>
            </option>
        ...
        <ul id="<?php echo esc_attr( $livedata_id ); ?>" class="covid-country-data">
          <li class="confirmedData">
            <div class="covtags-col-container">
              <span><?php echo $covtags_labels['cofirmed']; ?></span>
              <span><?php echo apply_filters( 'covtags_si_system_number_units', $wreport->cases, 'format_comma' );  ?></span>
              <span><?php echo esc_html__( 'Today:', COVTAGS_TEXTDOMAIN ); ?> ( <i>+<?php echo apply_filters( 'covtags_si_system_number_units', $wreport->todayCases, 'format_comma' );  ?></i> )</span>
            </div>
          </li>
          <li class="deathData">
            <div class="covtags-col-container">
              <span><?php echo $covtags_labels['deaths']; ?></span>
              <span><?php echo apply_filters( 'covtags_si_system_number_units', $wreport->deaths, 'format_comma' );  ?></span>
              <span><?php echo esc_html__( 'Today:', COVTAGS_TEXTDOMAIN ); ?> ( <i>+<?php echo apply_filters( 'covtags_si_system_number_units', $wreport->todayDeaths, 'format_comma' );  ?></i> )</span>
            </div>
          </li>
          <li class="criticalData">
            <div class="covtags-col-container">
              <span><?php echo $covtags_labels['critical']; ?></span>
              <span><?php echo apply_filters( 'covtags_si_system_number_units', $wreport->critical, 'format_comma' ); ?></span>
            </div>
          </li>
          <li class="recoveredData">
            <div class="covtags-col-container">
              <span><?php echo $covtags_labels['recovered']; ?></span>
              <span><?php echo apply_filters( 'covtags_si_system_number_units', $wreport->recovered, 'format_comma' ); ?></span>
            </div>
          </li>
        </ul>
      </div>
      ...
      <script type="text/javascript">
        window.applySelect2Plugin({
          _id       : <?php echo wp_json_encode( $unique_id ); ?>,
          countries : <?php echo wp_json_encode( $countries_data ); ?>,
          reportApi : <?php echo wp_json_encode( $wreport ); ?>
        });
      </script>
      <?php
      return ob_get_clean();
