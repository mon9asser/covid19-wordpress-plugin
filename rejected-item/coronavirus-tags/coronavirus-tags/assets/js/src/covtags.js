(function( $ ){
  ...
  /* Tricker Corona Data  */
  window.covtagsTrickerCoronaData = function ( trickerData ){

    var tricker      = jQuery( '#' + trickerData._id ),
        tirckers     = tricker.children( 'li' ),
        tickerWidth  = 0,
        trickNumber  = 0,
        tirckTimer   = trickerData.speed,
        currentSpace = 0,
        timer;
    tricker.css({ display: 'block' });

    // Default width
    tickerWidth = 30000;

    if( tirckers.lenght < 20 ){
      tickerWidth += 50;
      tirckers.each( function(){
        tickerWidth += jQuery( this ).outerWidth( true );
      });
    }

    // Assign width to parent element
    tricker.css({ width:tickerWidth });

    tricker.on( 'mouseover', function(){

      // Stop Tricker
       clearTimeout(timer);
       // Display Dropdown menu
       // jQuery( this ).children('.tooltip-data-container').show();
    });

    tricker.on( 'mouseleave', function(){
       startTricks();
    });

    // Trick items
    var trickItems = function (){

     if( parseInt( tricker.css( 'margin-left' ) )  ===  -tricker.eq(0).width()  ){
        tricker.css( 'margin-left', ( tricker.parent().width() ) + 'px' );
        trickNumber = tricker.parent().width() ;
     }

     tricker.css({ marginLeft:trickNumber });
     trickNumber--;
     startTricks();
    };

    // Start Timeframe
    var startTricks = function (){
